...
log(BLUE, `Starting installation of NotebookLM CLI for macOS...`);
...
if (isCommandPresent('uvx')) {
	...
	log(YELLOW, 'uvx not found. Checking for Homebrew to install uv...');
	if (isCommandPresent('brew')) {
		log(BLUE, 'Installing uv via Homebrew...');
		if (runCommand('brew install uv', 'Failed to install uv via brew.')) {
			installer = 'uv';
		}
	}

	if (!installer) {
		log(YELLOW, 'Falling back to universal alternative (pipx)...');
